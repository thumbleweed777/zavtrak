...
if( $_REQUEST['action'] == "edit" ) {
	$row = $db->super_query( "SELECT id, text, is_register, autor FROM " . PREFIX . "_comments where id = '$id'" );
...
{$bb_code}
<textarea name="dleeditcomments{$id}" id="dleeditcomments{$id}" onclick="setNewField(this.name, document.ajaxcomments{$id})" style="width:99%; height:150px;font-family:verdana; font-size:11px; border:1px solid #E0E0E0">{$comm_txt}</textarea><br>
<div align="right" style="width:99%;"><input class=bbcodes title="$lang[bb_t_apply]" type=button onclick="ajax_save_comm_edit('{$id}'); return false;" value="$lang[bb_b_apply]">
...
HTML;
	...
	$row = $db->super_query( "SELECT id, post_id, text, is_register, autor, approve FROM " . PREFIX . "_comments where id = '$id'" );
