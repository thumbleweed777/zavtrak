...
class Pagination {
	...
	function Pagination( $sql, $rows_per_page = 10, $links_per_page = 5, $append = "") {
        global $db;
	...
	#	$this->db->close();
		...
		$rs = $this->db->super_query($this->sql . " LIMIT {$this->offset}, {$this->rows_per_page}", true );
	 ...
	 * @return string
	 */
	function renderFirst($tag = 'Первая') {
		if ($this->total_rows == 0)
			return FALSE;
	 ...
	 * @return string
	 */
	function renderLast($tag = 'Последняя') {
		if ($this->total_rows == 0)
			return FALSE;
