...
while ( $row = $db->get_row( $sql_result ) ) {
	...
	if( $is_logged ) {
		...
		if( ! strstr( $member_id['favorites'], $row['id'] ) or $config['allow_cache'] == "yes" ) $tpl->set( '{favorites}', "<a href=\"$PHP_SELF?do=favorites&doaction=add&id=" . $row['id'] . "\"><img src=\"" . $config['http_home_url'] . "templates/{$config['skin']}/dleimages/plus_fav.gif\" title=\"" . $lang['news_addfav'] . "\" align=\"absmiddle\" border=\"0\"></a>" );
		else $tpl->set( '{favorites}', "<a href=\"$PHP_SELF?do=favorites&doaction=del&id=" . $row['id'] . "\"><img src=\"" . $config['http_home_url'] . "templates/{$config['skin']}/dleimages/minus_fav.gif\" title=\"" . $lang['news_minfav'] . "\" align=\"absmiddle\" border=\"0\"></a>" );
