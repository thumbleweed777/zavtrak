...
if( $cron == 2 ) {
	...
	$sql_cron = $db->query( "SELECT id, autor FROM " . PREFIX . "_post WHERE expires != '0000-00-00' AND expires < '" . date( "Y-m-d", $_TIME ) . "'" );
	...
	while ( $row = $db->get_row( $sql_cron ) ) {
		...
		$row_1 = $db->super_query( "SELECT images  FROM " . PREFIX . "_images where news_id = '{$row['id']}'" );
		...
		$getfiles = $db->query( "SELECT id, onserver FROM " . PREFIX . "_files WHERE news_id = '{$row['id']}'" );
