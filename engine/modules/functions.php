...
=====================================================
 Ôàéë: functions.inc.php
-----------------------------------------------------
 Íàçíà÷åíèå: Îñíîâíûå ôóíêöèè
=====================================================
*/
...
function check_module($aviable, $block, $do, $action = true) {
...
function news_sort($do) {
	...
	if( strtolower( $direction ) == "asc" ) {
		...
		$soft_by_array[$soft_by]['image'] = "<img src=\"{THEME}/dleimages/asc.gif\" />";
		$soft_by_array[$soft_by]['direction'] = "desc";
	
	} else {
		
		$soft_by_array[$soft_by]['image'] = "<img src=\"{THEME}/dleimages/desc.gif\" />";
	...
	$sort = "<form name=\"news_set_sort\" id=\"news_set_sort\" method=\"post\">" . $lang['sort_main'] . "&nbsp;" . implode( " | ", $sort );
