...
if( ! $tpl->result['content'] ) {
	...
	while ( $row = $db->get_row() ) {
		...
		if( $config['allow_alt_url'] == "yes" ) $user_name = "<a href=\"" . $config['http_home_url'] . "user/" . urlencode( $row['name'] ) . "/\">" . $row['name'] . "</a>";
		else $user_name = "<a href=\"$PHP_SELF?subaction=userinfo&amp;user=" . urlencode( $row['name'] ) . "\">" . $row['name'] . "</a>";
