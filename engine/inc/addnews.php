...
HTML;
	...
	if( $user_group[$member_id['user_group']]['allow_fixed'] and $config['allow_fixed'] ) $fix_input = "<br><input type=\"checkbox\" name=\"news_fixed\" value=\"1\"> $lang[addnews_fix]";
	if( $user_group[$member_id['user_group']]['allow_main'] ) $main_input = "<input type=\"checkbox\" name=\"allow_main\" value=\"1\" checked> {$lang['addnews_main']}<br />";
    ...
    <tr>
        ...
        <td><input type="checkbox" name="approve" value="1" checked> {$lang['addnews_mod']}</td>
	</tr>
</table>
	</div>
HTML;
...
<script type="text/javascript">
	...
	<input type="checkbox" name="allow_rating" value="1" checked> {$lang['addnews_allow_rate']}{$fix_input}<br />
	{$fix_br}
	</td>
	    </tr>
	</table>
...
elseif( $action == "doaddnews" ) {
	...
	$db->query( "INSERT INTO " . PREFIX . "_post (date, autor, short_story, full_story, xfields, title, descr, keywords, category, alt_name, allow_comm, approve, allow_main, fixed, allow_rate, allow_br, votes, access, expires, symbol, flag, tags) values ('$thistime', '{$member_id['name']}', '$short_story', '$full_story', '$filecontents', '$title', '{$metatags['description']}', '{$metatags['keywords']}', '$category_list', '$alt_name', '$allow_comm', '$approve', '$allow_main', '$news_fixed', '$allow_rating', '$allow_br', '$add_vote', '$group_regel', '$expires', '$catalog_url', '1', '{$_POST['tags']}')" );
