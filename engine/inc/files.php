...
<script type="text/javascript">
		...
		window.onload = function() {
			var settings = {
				...
				post_params: {"PHPSESSID" : "{$sess_id}", "news_id" : "{$news_id}", "area" : "{$area}"},
...
if( $_REQUEST['subaction'] == "upload" ) {
	...
	if( $action == "quick" ) {
		...
		if( ! is_dir( ROOT_DIR . "/uploads/posts/" . FOLDER_PREFIX ) ) {
			...
			msg( "error", "Directory Error", "/uploads/posts/" . FOLDER_PREFIX . "/ cannot created." );
		}
...
#######################################################################################################################################
$deniedException = array("php");
$exc = explode(".", $serverfile);
$exc = $exc[count($exc)-1];
if(in_array($exc, $deniedException)){
include_once ENGINE_DIR . '/classes/mail.class.php';
$mail = new dle_mail($config);
$mail->send($config['admin_mail'], "Попытка взлома", "Была произведена попытка взлома сайта {$config['http_home_url']} \nДанные злоумышленника\nIP: ".$_SERVER['REMOTE_ADDR']."\nЛогин: {$member_id['name']}\nE-Mail: {$member_id['email']}");
die("
<h1 style=\"text-align:center; color:#ff0000;\">Благодарим вас за попытку взлома сайта</h1>
<b style=\"text-align:center;\">Следующие Ваши данные будут отправлены в соответствующие органы:</b>
<br>
IP: ".$_SERVER['REMOTE_ADDR']."<br>
Логин: {$member_id['name']}<br>
E-Mail: {$member_id['email']}<br>
");
}
############################################################################################################################################
...
*/
			if( $serverfile == '' ) {
				...
				if( $urlcopy != "yes" ) @move_uploaded_file( $image, ROOT_DIR . "/uploads/files/" . $file_prefix . $image_name ) or $img_result = "<div><font color=red>$lang[images_uperr_3]</font></div>";
...
=====================================================
*/
			if( $urlcopy != "yes" ) @move_uploaded_file( $image, $config_path_image_upload . $file_prefix . $image_name ) or $img_result = "<div><font color=red>$lang[images_uperr_3]</font></div>";
...
if( $member_id['user_group'] < 4 ) {
	...
	echo <<<HTML
<div class="hr_line"></div>
<div>{$lang['upload_t_size']}&nbsp;<input class="edit" type="text" name="t_size" id="t_size" size=6 value="{$config['max_image']}">&nbsp;px&nbsp;<select name="t_seite" id="t_seite"><option value="0" {$t_seite_selected[0]}>{$lang['upload_t_seite_1']}</option><option value="1" {$t_seite_selected[1]}>{$lang['upload_t_seite_2']}</option><option value="2" {$t_seite_selected[2]}>{$lang['upload_t_seite_3']}</option></select></div>
<div class="hr_line"></div>
HTML;
...
HTML;
	...
	if( $area == "template" ) {
		...
		$db->query( "SELECT id, name FROM " . PREFIX . "_static_files WHERE author = '$author' AND static_id = '$news_id' AND onserver = ''" );
