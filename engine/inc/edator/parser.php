...
foreach ($categories as $cat) {
    ...
    #var_dump($class->getPageLinks()); die;

    foreach ($class->getPageLinks() as $link) {

        if (code_exist(md5($link))) continue;
        #add_code($cat['id'], md5($link));
        #var_dump($class->getPostLinks($link)); die;

        foreach ($class->getPostLinks($link) as $pLink) {
            ...
            foreach ($post as $vv) {
                ...
                if ($vv == false || strlen(trim(strip_tags($vv))) < 10 || $vv == null)
                    continue 2;
            }

            if (post_exist($post['title'])) {
                echo $post['title'] .  "- äóáëü <br>";
                continue;
            }
...
function get_class_name($url)
{
    ...
    if (preg_match('#gotovim-sami#', $url)) $name = 'GotovimSamiParserCore';
    elseif (preg_match('#cook-note#', $url)) $name = 'CookNoteParserCore'; elseif (preg_match('#menagere\.ru#', $url)) $name = 'MenagereRuParserCore'; elseif (preg_match('#gotovo-doma#', $url)) $name = 'GotovoDomaParserCore'; elseif (preg_match('#mapcook\.ru#', $url)) $name = 'MapcookRuParserCore'; elseif (preg_match('#foodquiz\.ru#', $url)) $name = 'FoodquizRuParserCore';
