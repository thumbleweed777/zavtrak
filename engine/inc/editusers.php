...
<table width="100%">
    ...
    <tr>
        <td>&nbsp;</td>
        <td style="padding-top:10px;"><input type="submit" class="buttons" value="{$lang['vote_new']}">
...
<script type="text/javascript">
    ...
    <tr>
        <td>&nbsp;</td>
        <td style="padding-top:10px;"><input type="submit" class="buttons" value=" {$lang['b_find']} ">
        <td style="padding-top:10px;"><input type="button" class="buttons" value=" {$lang['user_breset']} " onclick="javascript:clearform(document.searchform); return false;">
        <td colspan="4" style="padding-top:10px;"><input type="reset" class="buttons" value=" {$lang['user_brestore']} ">
...
<table width="100%">
    <tr>
        ...
        <td width="2">&nbsp;</td>
        <td width="90">{$lang['user_news']}</td>
        <td width="90">{$lang['user_coms']}</td>
        <td width="112" align="center">{$lang['user_acc']}</td>
        <td width="153">{$lang['user_action']}</td>
    </tr>
...
HTML;
	...
	while ( $row = $db->get_row() ) {
        ...
        &nbsp;$user_level</td>
        <td width=153 title='' class='list' nowrap='nowrap'><nobr>
        ...
        </tr><tr><td background=\"engine/skins/images/mline.gif\" height=1 colspan=9></td></tr>";
	}
	$db->free();
...
elseif( $action == "adduser" ) {
	...
	if( preg_match( "/[\||\'|\<|\>|\"|\!|\$|\@|\&\~\*\+]/", $regusername ) ) msg( "error", $lang['user_err'], $lang['user_err_1'], "javascript:history.go(-1)" );
	if( ! $regpassword ) {
...
<option value="3" $restricted_selected[3]>{$lang['restricted_all']}</option>
    ...
    <tr>
        ...
        <td colspan="2"><input type="file" name="image" style="width:304px; height:18px" class="edit" /></td>
    </tr>
    <tr>
    ...
    <tr>
        ...
        <td colspan="2"><textarea style="width:100%; height:70px;" name="editinfo">{$row['info']}</textarea></td>
    </tr>
    <tr>
	...
	{$output}
    <tr>
        <td colspan="3" style="padding-top:10px;padding-bottom:10px;"><input type="submit" value="{$lang['user_save']}" class="buttons">   <input type="button" value="{$lang['user_can']}" onClick="window.close();" class="buttons">
...
elseif( $action == "doedituser" ) {
	...
	$editicq = $db->safesql( $parse->process( $_POST['editicq'] ) );
...
elseif( $action == "dodeleteuser" ) {
	...
	$row = $db->super_query( "SELECT name, foto FROM " . USERPREFIX . "_users WHERE user_id='$id'" );
