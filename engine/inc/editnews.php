...
if( $action == "list" ) {
	...
	$db->query( "SELECT id, date, title, category, autor, alt_name, comm_num, approve, fixed, flag FROM " . PREFIX . "_post" . $where . " ORDER BY " . $order_by . " LIMIT $start_from,$news_per_page" );
...
elseif( $action == "editnews" ) {
	...
	if( $member_id['user_group'] == 1 ) {
		...
		$author_info = "<input type=\"text\" name=\"new_author\" size=\"20\"  class=edit value=\"{$row['autor']}\"><input type=\"hidden\" name=\"old_author\" value=\"{$row['autor']}\" />";
	
	} else {
		
		$author_info = "<a href=\"?mod=editusers&action=list&search=yes&search_name=" . urlencode( $row['autor'] ) . "\"><b>{$row['autor']}</b></a>";
...
HTML;
	...
	if( $user_group[$member_id['user_group']]['allow_fixed'] and $config['allow_fixed'] ) $fix_input = "<br><input type=\"checkbox\" name=\"news_fixed\" value=\"1\" $iffix> $lang[addnews_fix]";
	if( $user_group[$member_id['user_group']]['allow_main'] ) $main_input = "<input type=\"checkbox\" name=\"allow_main\" value=\"1\" {$ifmain}> {$lang['addnews_main']}<br />";
    ...
    <tr>
        ...
        <td><input type="checkbox" name="approve" value="1" {$ifapp}> {$lang['addnews_mod']}</td>
	</tr>
</table></div>
HTML;
...
<script type="text/javascript">
    ...
    <tr>
	    ...
	    <tr>
	        <td>&nbsp;</td>
	        <td>{$main_input}
		<input type="checkbox" name="allow_comm" value="1" {$ifch}> {$lang['addnews_comm']}<br />
		<input type="checkbox" name="allow_rating" value="1" {$ifrat}> {$lang['addnews_allow_rate']}
		{$fix_input}<br />
		{$fix_br}	
		</td>
	    </tr>
		...
		if( $have_perm ) {
			...
			if( $ifdelete != "yes" ) {
				...
				if( $member_id['user_group'] == 1 and $_POST['new_author'] != $_POST['old_author'] ) {
					...
					$row = $db->super_query( "SELECT user_id  FROM " . PREFIX . "_users WHERE name = '{$_POST['new_author']}'" );
