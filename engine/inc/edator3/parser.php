...
foreach ($categories as $cat) {
    ...
    $cc = $cat['v_count'];
    $className = get_class_name($cat['query']);
    if (!class_exists($className)) {
        echo "Class {$className} for this url not found.  <br>";
        continue;
    }
  ...
  #  var_dump($pageLinks); die;
    ...
    foreach ($pageLinks as $pK => $link) {
       ...
       add_code($cat['id'], md5($link));
        ...
        #var_dump($pageLinks, $postLinks); die;
        ...
        foreach ($postLinks as $postK => $pLink) {
            ...
            if (code_exist(md5($pLink))) {
                echo $pLink .   " - óæå åñòü<br>";
                continue;
            }
