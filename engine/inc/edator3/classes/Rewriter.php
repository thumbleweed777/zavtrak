...
class Rewriter
{
    ...
    public function __construct($filename)
    {
        ...
        $file = file_get_contents(MODULE_DIR . "/synonyms/" . $filename);


        preg_match_all('#\[w\]([^\[]+)\[/w\]\[w\]([^\[]+)\[/w\]#xmsi', $file, $s_arr, PREG_SET_ORDER);
        $this->s_arr = $s_arr;
...
//Замена "шт"
    protected function rewrite_sht($text)
    {
        ...
        foreach ($array as $val) {
            ...
            if (!isset($repleace)) $repleace = " {$val[1]} штук ";

            $replace_text = '#' . preg_quote($val[0]) . '#mi';

            $text = preg_replace($replace_text, $repleace, $text);
            $text = str_ireplace(' ук ук', '', $text);
        }
    ...
    protected function rewrite_litry($text)
    {
        preg_match_all('#(((\d+(,|\.|-))?)\s*(\d+))\s*л((\.)?)#msi', $text, $array, PREG_SET_ORDER);
