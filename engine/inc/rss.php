...
if( $_REQUEST['action'] == "addnews" ) {
	...
	if( count( $_POST['content'] ) ) {
		...
		foreach ( $_POST['content'] as $content ) {
			...
			$category_list = $db->safesql( implode( ',', $content['category'] ) );
...
<script language="javascript" type="text/javascript">
	...
	function doFull( link, news_id, rss_id )
	{
		...
		var varsString = "link=" + link;

		ajax.setVar("news_id", news_id);
		ajax.setVar("rss_id", rss_id);
		ajax.setVar("rss_charset", '{$xml->rss_charset}');

		ajax.requestFile = "engine/ajax/rss.php";
		ajax.element = 'cfull' + news_id;
		ajax.method = 'POST';
...
HTML;
    ...
    <b><a onClick="RemoveTable(' . $i . ')" href="#" ><img src="engine/skins/images/rss_delete.gif"  align="absmiddle" style="border:0;" /></a> <a class="main" href="javascript:ShowOrHide(\'cp' . $i . '\',\'cc' . $i . '\')" >' . $content['title'] . '</a></td>
    </tr>
    <tr id=\'cp' . $i . '\' style=\'display:none\'>
    <td width=200 valign="top" style="padding: 5px"><input class="edit" type="text" size="55" id="title_' . $i . '" name="content[' . $i . '][title]" value="' . $content['title'] . '"><br />
	<br /><input type="text" name="content[' . $i . '][date]" id="f_date_c' . $i . '" size="20"  class=edit value="' . $content['date'] . '">
...
<script type="text/javascript">
    ...
    <td colspan="2">
    <textarea rows="10" style="width:600px;" id="short_' . $i . '" name="content[' . $i . '][short]">' . $content['description'] . '</textarea>
	...
	<br /><input onClick="doFull(\'' . urlencode( rtrim( $content['link'] ) ) . '\', \'' . $i . '\', \'' . $rss['id'] . '\')" type="button" class="buttons" value="' . $lang['rss_dofull'] . '">&nbsp;&nbsp;<input onClick="preview(' . $i . ')" type="button" class="buttons" value="' . $lang['btn_preview'] . '">&nbsp;&nbsp;<input onClick="RemoveTable(' . $i . ')" type="button" class="buttons" value="' . $lang['edit_dnews'] . '"><br /><br />
...
<table width="100%">
    <tr>
        ...
        <td style="padding-top:2px;padding-bottom:2px;"><input class="edit" type="text" size="25" name="rss_url" value="{$rss_url_value}"><a href="#" class="hintanchor" onMouseover="showhint('{$lang[rss_hurl]}', this, event, '220px')">[?]</a></td>
    </tr>
	<tr><td background="engine/skins/images/mline.gif" height=1 colspan=2></td></tr>
    <tr>
        <td style="padding:4px;">{$lang['rss_descr']}</td>
        <td style="padding-top:2px;padding-bottom:2px;"><input class="edit" type="text" size="55" name="rss_descr" value="{$rss_descr_value}"><a href="#" class="hintanchor" onMouseover="showhint('{$lang[rss_hdescr]}', this, event, '220px')">[?]</a></td>
    </tr>
	<tr><td background="engine/skins/images/mline.gif" height=1 colspan=2></td></tr>
    <tr>
        <td style="padding:4px;">{$lang['rss_maxnews']}</td>
        <td style="padding-top:2px;padding-bottom:2px;"><input class="edit" type="text" size="5" name="rss_maxnews" value="{$rss_maxnews_value}"><a href="#" class="hintanchor" onMouseover="showhint('{$lang[rss_hmaxnews]}', this, event, '220px')">[?]</a></td>
    </tr>
	<tr><td background="engine/skins/images/mline.gif" height=1 colspan=2></td></tr>
    <tr>
