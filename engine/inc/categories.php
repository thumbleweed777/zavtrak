...
if( $action == "add" ) {
	...
	$cat_icon = htmlspecialchars( $db->safesql( $_POST['cat_icon'] ) );
	...
	$description = $db->safesql( substr( strip_tags( stripslashes( $_REQUEST['descr'] ) ), 0, 150 ) );
	$keywords = $db->safesql( str_replace( $quotes, " ", strip_tags( stripslashes( $_REQUEST['keywords'] ) ) ) );
	...
	if( $row['count'] ) {
		...
		if( is_array( $_REQUEST['new_category'] ) ) {
			if( ! in_array( $catid, $new_category ) ) {
				...
				$category_list = $db->safesql( implode( ',', $_REQUEST['new_category'] ) );
...
<table width="100%">
    ...
    <tr>
	    ...
	    <tr>
	        ...
	        <td><input type="text" name="descr" size="55" value="{$row['descr']}" class="edit"> ({$lang['meta_descr_max']})</td>
	    </tr>
	    <tr>
...
<table width="100%">
    ...
    <tr>
	    ...
	    <tr>
	        <td style="padding:4px;">{$lang['meta_descr_cat']}</td>
	        <td><input type="text" name="descr" size="55"  class="edit"> ({$lang['meta_descr_max']})</td>
	    </tr>
	    <tr>
	        <td style="padding:4px;">{$lang['meta_keys']}</td>
