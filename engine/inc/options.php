...
if( $action == "options" or $action == '' ) {
	...
	$options['others'] = array (
        ...
        array(
            'name' => 'Тагинатор',
            'url' => "$PHP_SELF?mod=taginator",
            'descr' => 'управление созданными тэгами для новостей',
            'image' => "taginator.png",
            'access' => "1"
        ),
...
'url' => "$PHP_SELF?mod=referer",
'descr' => 'Учёт совершенных переходов на Ваш сайт. Просмотр поисковых запросов с поисковых систем.',
'image' => "referer.png",
'access' => "1",
),
...
array(
            'name' => 'Видеонатор',
            'url' => "$PHP_SELF?mod=videonator",
            'descr' => 'Автоматический парсер видео на сайт',
            'image' => "youtube.png",
            'access' => "1"
        ),
	...
	foreach ( $options as $sub_options => $value ) {
		...
		for($i = 0; $i < $count_options; $i ++) {
			if( $member_id['user_group'] > $value[$i]['access'] and $value[$i]['access'] != "all" ) {
